...
tx = abs(fft(up_signal));
rx = abs(fft(received_signal));
tx_norm = tx(1:100) / max(tx(1:100));
rx_norm = rx(1:100) / max(rx(1:100));

plot(tx, 'b-', 'LineWidth', 2, 'DisplayName', 'Передаваемый');
hold on;
plot( rx, 'r--', 'LineWidth', 1.5, 'DisplayName', 'Принимаемый');
title('Сравнение спектров при N = 5');
...
sigma_values = 0:0.1:2; % Диапазон значений σ от 0 до 1 с шагом 0.1
corr_peaks = zeros(1, length(sigma_values)); % Массив для пиков корреляции

for idx = 1:length(sigma_values)
    sigma = sigma_values(idx);
    
    % Генерируем шум с текущим σ
    noise = sigma * randn(1, length(reserved));
    received_signal = reserved + noise;
    ...
    [corr_max, ~] = max(norm_corr);
    corr_peaks(idx) = corr_max;
end   

%% График зависимости пика корреляции от σ
figure('Position', [100 100 800 400]);

plot(sigma_values, corr_peaks, 'b-o', 'LineWidth', 2, 'MarkerSize', 8, 'MarkerFaceColor', 'b');
title('Зависимость пика корреляции от σ');
xlabel('σ (уровень шума)');
