...
norm = correlation / sqrt(sum(a1.^2) * sum(b1.^2)) ;
...
%for shift = 0:6
 ...
 %   corr = sum(a1 .* b_shift);% Считаем корреляцию: умножаем элементы a и b_shift и складываем
  %  normc = corr / sqrt(sum(a1.^2) * sum(b1.^b_shift)) ;
   % results = [results, normc];
%end
...
for shift = 0:6
    ...
    b_shift = zeros(1, n);
    for i = 1:n
        new_pos = mod(i - 1 - shift, n) + 1;
        b_shift(i) = b1(new_pos);
    end
    
    corr = sum(a1 .* b_shift); % Считаем корреляцию
    normc = corr / sqrt(sum(a1.^2) * sum(b_shift.^2)); % Исправлено: b_shift вместо b1
    results = [results, normc];
end
